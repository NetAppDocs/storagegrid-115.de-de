...
IMPORTANT: Wenn Sie Ports neu zuordnen, können Sie nicht dieselben Ports zum Konfigurieren von Load Balancer-Endpunkten verwenden. Wenn Sie Load Balancer-Endpunkte konfigurieren und bereits neu zugeordnete Ports haben möchten, befolgen Sie die Schritte in den Recovery- und Wartungsanweisungen zum Entfernen von Port-Remaps.



.Schritte
. Klicken Sie in der Menüleiste des Installationsprogramms für StorageGRID-Geräte auf *Netzwerke konfigurieren* > *Ports für die Erinnerung*.
+
